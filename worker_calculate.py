...
DEFAULT_LOGS_DIR = worker.WorkerConfig().DEFAULT_LOGS_DIR
...
if __name__ == '__main__':
    ...
    # variables
    decimal_place = 8
    model_name = 'mask_rcnn_worker_'
    save_csv_dir = '../drive/My Drive/silhouette_weight/'
    ...
    if mode == 'original':
        ...
        wd = []
        temp_save_wd = save_csv_dir + folder_name + '_wd_temp.csv'
        if start != 1:
            temp_size = start - 1
        ...
        for i in range(start, m_amount):
            last_model = load_weight(last_model_path, worker.WorkerConfig())
        ...
        # velocity
        epoch = 1
        save_v_path = save_csv_dir + folder_name + '_v.csv'
        v = []
